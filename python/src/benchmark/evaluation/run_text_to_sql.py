...
from benchmark.evaluation.plots import CAUSE_ORDER, plot_cause_breakdown, plot_miss_rate_bars
from benchmark.question_registry import QuestionSpec, load_questions


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate text-to-SQL outputs with outcome-focused fragmentation metrics."
    )
    parser.add_argument("--run-dir", type=Path, default=Path("artifacts/runs/local"))
    parser.add_argument("--questions-file", type=Path, help="Question registry JSON/TXT file.")
    parser.add_argument(
        "--generated-results-dir",
        type=Path,
        help="Directory containing text-to-SQL generated result CSVs.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        help="Output directory for evaluation metrics, figures, and reports.",
    )
    parser.add_argument("--query-id", action="append", default=[], help="Optional question_id filter.")
    parser.add_argument(
        "--plot-format",
        default="png,pdf",
        help="Comma-separated output plot formats. Example: png,pdf",
    )
    parser.add_argument("--strict", action="store_true", help="Fail instead of warning on missing inputs.")
...
def main() -> None:
    ...
    if args.query_id:
        ...
        if not questions:
            raise ValueError("No questions matched --query-id")

    generated_results_dir = args.generated_results_dir or (
    ...
    plot_formats = [part.strip() for part in args.plot_format.split(",") if part.strip()]
    if not plot_formats:
        raise ValueError("--plot-format must contain at least one format")
...
def evaluate_single_question(
    *,
    ...
    baseline_path = generated_result_path(generated_results_dir, question.question_id, baseline_variant)
    ...
    for variant in VARIANT_NAMES:
        result_path = generated_result_path(generated_results_dir, question.question_id, variant)
        if not result_path.exists():
            if strict:
                raise FileNotFoundError(f"Missing generated result for {question.question_id}: {result_path}")
            ...
            missed_rows.append(
                {
                    ...
                    "baseline_weight": weight_lookup.get(student_id, 1.0) if weight_lookup else "",
    ...
    for plot_format in plot_formats:
        plot_miss_rate_bars(
            metrics_rows,
            figures_dir / f"{safe_slug(question.question_id)}__main_miss_rate.{plot_format}",
            title=question.display_title,
        )

    if any(
        row.get(cause, 0)
        for row in cause_plot_rows
        for cause in CAUSE_ORDER
    ):
        for plot_format in plot_formats:
            plot_cause_breakdown(
                cause_plot_rows,
                figures_dir / f"{safe_slug(question.question_id)}__cause_breakdown.{plot_format}",
                title=f"{question.display_title}: Why Students Were Missed",
            )

    summary_text = build_question_summary(question, metrics_rows)
    summary_path = reports_dir / f"{safe_slug(question.question_id)}__summary.md"
    caption_path = reports_dir / f"{safe_slug(question.question_id)}__presentation_caption.txt"
    summary_path.write_text(summary_text + "\n", encoding="utf-8")
    caption_path.write_text(build_presentation_caption(question, metrics_rows) + "\n", encoding="utf-8")
...
def maybe_write_generic_plot_aliases(
    *,
    ...
    if not single_query:
        return
    for plot_format in plot_formats:
        source = figures_dir / f"{safe_slug(question.question_id)}__main_miss_rate.{plot_format}"
        if source.exists():
            target = figures_dir / f"main_miss_rate.{plot_format}"
            target.write_bytes(source.read_bytes())


def generated_result_path(generated_results_dir: Path, question_id: str, variant: str) -> Path:
    return generated_results_dir / f"{safe_slug(question_id)}__{safe_slug(variant)}.csv"
...
def assign_missing_causes(missing_ids: list[str], manifest: dict[str, Any]) -> dict[str, str]:
...
def build_question_summary(question: QuestionSpec, metrics_rows: list[dict[str, Any]]) -> str:
...
def build_presentation_caption(question: QuestionSpec, metrics_rows: list[dict[str, Any]]) -> str:
